...
class InfLoRA_b5(Finetune):
    ...
    def observe(self, data):
        ...
        # y = y - self.known_classes

        # mask = (targets >= self._known_classeses).nonzero().view(-1)
        # inputs = torch.index_select(inputs, 0, mask)
        # targets = torch.index_select(targets, 0, mask) - self._known_classeses
    ...
    def inference(self, data):
        ...
        # outputs_with_task = torch.zeros_like(logits)[:, :self.class_num]
        # # 遍历每个任务
        # for idx, i in enumerate(y // self.class_num):
        #     en, be = self.class_num * i, self.class_num * (i + 1)
        #     outputs_with_task[idx] = logits[idx, en:be]
        #
        # predicts_with_task = outputs_with_task.argmax(dim=1)
        # predicts_with_task = predicts_with_task + (y // self.class_num) * self.class_num
